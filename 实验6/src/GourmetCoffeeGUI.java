...
public class GourmetCoffeeGUI extends JPanel {
	...
	class AddModifyListener implements ActionListener
	{
		...
		public void actionPerformed(ActionEvent event)
		{
			...
			catch (NumberFormatException numberFormatException)//数字格式的错误
			{
	...
	class RemoveListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent event)
		{
			...
			else if(orderList.getSelectedValue()==null)
			{
				statusTextArea.setText("You have not selected an item in the order.");
				return;
			}

			if (!orderList.getValueIsAdjusting())
			{
				Product selectedProduct = ((OrderItem) orderList.getSelectedValue()).getProduct();
				for (Iterator<OrderItem> orderItemIterator = currentOrder.iterator(); orderItemIterator.hasNext(); )
				{
					OrderItem currentOrderItem = orderItemIterator.next();
					if (currentOrderItem.getProduct().equals(selectedProduct))
					{
	...
	class SaveSalesListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent event)
		{
			...
			if (fileSelectionResult == JFileChooser.APPROVE_OPTION)//文件选择成功
			{
				...
				{
					statusTextArea.setText("The file specified cannot be created or opened.");
				}
			}
			else if (fileSelectionResult == JFileChooser.CANCEL_OPTION)
			{
